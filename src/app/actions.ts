...
import { generateRefinementSuggestions, type GenerateRefinementSuggestionsInput, type GenerateRefinementSuggestionsOutput } from '@/ai/flows/generate-refinement-suggestions';
...
/**
 * Server action to refine a prompt using the Genkit flow.
 * @param data - The input for prompt refinement, including original prompt and refinement instructions.
 * @returns An object with the refined prompt data or an error message.
 */
export async function refinePromptAction(
  data: RefinePromptInput
): Promise<{ data?: RefinePromptOutput; error?: string }> {
  try {
    const result = await refinePrompt(data);
    if (!result) {
      return { error: 'The AI service did not return a response. Please try again.' };
    }
    return { data: result };
...
/**
 * Server action to generate refinement suggestions using the Genkit flow.
 * @param data - The input for refinement suggestion generation.
 * @returns An object with the generated refinement suggestions or an error message.
 */
export async function generateRefinementSuggestionsAction(
  data: GenerateRefinementSuggestionsInput
): Promise<{ data?: GenerateRefinementSuggestionsOutput; error?: string }> {
  try {
    const result = await generateRefinementSuggestions(data);
    if (!result) {
      return { error: 'The AI service did not return a response. Please try again.' };
    }
    return { data: result };
  } catch (error) {
    console.error('Error during refinement suggestion generation:', error);
    return { error: `An unexpected error occurred: ${error instanceof Error ? error.message : String(error)}` };
  }
}
